...
public sealed class RoleApiService
{
    ...
    public RoleApiService(IHttpClientFactory httpClientFactory, AppState appState, AuthState authState)
    {
    ...
    public async Task<CatalogViewDefinition> GetCatalogAsync()
    {
        ...
        var rolesTask = client.GetFromJsonAsync<List<RoleRowDto>>("/api/security/roles");
        var tenantsTask = client.GetFromJsonAsync<List<RoleTenantLookupDto>>("/api/security/roles/tenants");
        await Task.WhenAll(rolesTask!, tenantsTask!);
        ...
        {
            new() { DataField = "RoleId", Caption = "Role ID", DataType = "string", AllowEditing = false, Width = 220, Visible = false },
            ...
            new() { DataField = "Code", Caption = "Código", DataType = "string", Required = true, Width = 140 },
            new() { DataField = "Name", Caption = "Rol", DataType = "string", Required = true, Width = 220 },
            new() { DataField = "Description", Caption = "Descripción", DataType = "string", Width = 260 },
            new() { DataField = "IsSystemRole", Caption = "Rol sistema", DataType = "boolean", Width = 120 },
            new() { DataField = "IsActive", Caption = "Activo", DataType = "boolean", Width = 90 }
        };

        var rows = roles.Select(x => new Dictionary<string, object?>
        {
        ...
        return new CatalogViewDefinition
        {
            CatalogKey = "roles",
            Title = "Roles",
            Subtitle = _authState.IsPlatformOwner ? "Administración real de roles desde API + PostgreSQL." : $"Roles del tenant activo: {_appState.CurrentTenantName}.",
            KeyExpr = "RoleId",
            AllowCreate = true,
            AllowUpdate = true,
            AllowDelete = true,
    ...
    public async Task<CatalogViewDefinition> InsertAsync(JsonElement payload)
    {
        ...
        var client = _httpClientFactory.CreateClient("Nanchesoft.Api");
        var response = await client.PostAsJsonAsync("/api/security/roles", request);
    ...
    public async Task<CatalogViewDefinition> UpdateAsync(string key, JsonElement payload)
    {
        var request = MapRequest(payload);
        var client = _httpClientFactory.CreateClient("Nanchesoft.Api");
        var response = await client.PutAsJsonAsync($"/api/security/roles/{key}", request);
        await EnsureSuccessAsync(response);
        return await GetCatalogAsync();
    }
    ...
    {
        var client = _httpClientFactory.CreateClient("Nanchesoft.Api");
        var response = await client.DeleteAsync($"/api/security/roles/{key}");
        await EnsureSuccessAsync(response);
        return await GetCatalogAsync();
    }

    private CreateOrUpdateRoleRequest MapRequest(JsonElement payload)
    {
        var request = new CreateOrUpdateRoleRequest
        {
            ...
            IsSystemRole = ReadBool(payload, "IsSystemRole", false),
    ...
    private List<RoleRowDto> FilterRoles(List<RoleRowDto> rows)
    {
        if (_authState.IsPlatformOwner || !_appState.CurrentTenantId.HasValue)
    ...
    private List<RoleTenantLookupDto> FilterTenants(List<RoleTenantLookupDto> rows)
    {
        if (_authState.IsPlatformOwner || !_appState.CurrentTenantId.HasValue)
