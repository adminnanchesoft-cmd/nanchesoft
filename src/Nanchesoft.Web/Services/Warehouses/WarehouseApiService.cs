...
namespace Nanchesoft.Web.Services.Warehouses;
...
public sealed class WarehouseApiService
{
    ...
    public WarehouseApiService(IHttpClientFactory httpClientFactory, AppState appState, AuthState authState)
    {
    ...
    public async Task<CatalogViewDefinition> GetCatalogAsync()
    {
        ...
        var warehousesTask = client.GetFromJsonAsync<List<WarehouseRowDto>>("/api/organization/warehouses");
        var branchesTask = client.GetFromJsonAsync<List<WarehouseBranchLookupDto>>("/api/organization/warehouses/branches");
        await Task.WhenAll(warehousesTask!, branchesTask!);

        var warehouses = FilterWarehouses(warehousesTask.Result ?? []);
        ...
        {
            new() { DataField = "WarehouseId", Caption = "Warehouse ID", DataType = "string", AllowEditing = false, Width = 220, Visible = false },
            new() { DataField = "TenantName", Caption = "Tenant", DataType = "string", AllowEditing = false, Width = 180, Visible = showTenantColumn },
            new() { DataField = "BranchId", Caption = "Sucursal", DataType = "string", Required = true, Width = 220, UseLookup = true, LookupItems = branches.Select(x => new CatalogLookupItem { Id = x.BranchId.ToString("D"), Name = x.BranchName }).ToList() },
            new() { DataField = "Code", Caption = "Código", DataType = "string", Required = true, Width = 110 },
            new() { DataField = "Name", Caption = "Almacén", DataType = "string", Required = true, Width = 220 },
            new() { DataField = "IsActive", Caption = "Activo", DataType = "boolean", Width = 90 }
        };

        var rows = warehouses.Select(x => new Dictionary<string, object?>
        {
            ["WarehouseId"] = x.WarehouseId.ToString("D"),
        ...
        return new CatalogViewDefinition
        {
            ...
            Title = "Almacenes",
            Subtitle = _authState.IsPlatformOwner ? "Administración real de almacenes desde API + PostgreSQL." : $"Almacenes del tenant activo: {_appState.CurrentTenantName}.",
            KeyExpr = "WarehouseId",
            AllowCreate = true,
            AllowUpdate = true,
            AllowDelete = true,
    ...
    public async Task<CatalogViewDefinition> InsertAsync(JsonElement payload)
    {
        ...
        var client = _httpClientFactory.CreateClient("Nanchesoft.Api");
        var response = await client.PostAsJsonAsync("/api/organization/warehouses", request);
    ...
    public async Task<CatalogViewDefinition> UpdateAsync(string key, JsonElement payload)
    {
        var request = MapRequest(payload);
        var client = _httpClientFactory.CreateClient("Nanchesoft.Api");
        var response = await client.PutAsJsonAsync($"/api/organization/warehouses/{key}", request);
        await EnsureSuccessAsync(response);
        return await GetCatalogAsync();
    }
    ...
    {
        var client = _httpClientFactory.CreateClient("Nanchesoft.Api");
        var response = await client.DeleteAsync($"/api/organization/warehouses/{key}");
        await EnsureSuccessAsync(response);
        return await GetCatalogAsync();
    }

    private List<WarehouseRowDto> FilterWarehouses(List<WarehouseRowDto> rows)
    {
        ...
        return filtered.OrderBy(x => x.Name, StringComparer.OrdinalIgnoreCase).ToList();
    }

    private List<WarehouseBranchLookupDto> FilterBranches(List<WarehouseBranchLookupDto> rows)
    {
        var filtered = rows.AsEnumerable();
        if (!_authState.IsPlatformOwner && _appState.CurrentTenantId.HasValue)
            filtered = filtered.Where(x => x.TenantId == _appState.CurrentTenantId.Value);
        if (!_authState.IsPlatformOwner && _appState.CurrentCompanyId.HasValue)
            filtered = filtered.Where(x => x.CompanyId == _appState.CurrentCompanyId.Value);
        return filtered.OrderBy(x => x.BranchName, StringComparer.OrdinalIgnoreCase).ToList();
    }

    private static CreateOrUpdateWarehouseRequest MapRequest(JsonElement payload) => new()
    {
...
public sealed class WarehouseRowDto
{
