...
public sealed class TreasuryExpenseConfiguration : IEntityTypeConfiguration<TreasuryExpense>
{
    public void Configure(EntityTypeBuilder<TreasuryExpense> builder)
    {
        builder.ToTable("treasury_expenses");
        ...
        builder.HasMany(x => x.Lines).WithOne(x => x.TreasuryExpense).HasForeignKey(x => x.TreasuryExpenseId).OnDelete(DeleteBehavior.Cascade);
    }
}
