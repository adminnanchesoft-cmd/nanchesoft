...
public static class PayrollCalculatedEndpoints
{
    ...
    private static async Task<IResult> CreatePayrollSourceApplicationAsync(HttpContext httpContext, PayrollSourceApplicationRequest request, NanchesoftDbContext db)
    {
    ...
    private static async Task<IResult> CreatePayrollReceiptControlAsync(HttpContext httpContext, PayrollReceiptControlRequest request, NanchesoftDbContext db)
    {
        var context = await ResolveDefaultContextAsync(httpContext, db);
        var tenantId = request.TenantId ?? context.TenantId;
        var companyId = request.CompanyId ?? context.CompanyId;
    ...
    private static async Task<IResult> CreatePayrollRunClosingAsync(HttpContext httpContext, PayrollRunClosingRequest request, NanchesoftDbContext db)
    {
        var context = await ResolveDefaultContextAsync(httpContext, db);
        var tenantId = request.TenantId ?? context.TenantId;
        var companyId = request.CompanyId ?? context.CompanyId;
